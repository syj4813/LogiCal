...
def explain_delay_risk(probability: float, level: str, signals: dict, weather_summary: str | None = None) -> str:
    ...
    weather_bullet = ""
    seasonal_bullets = (
        '  - 장마철 해당 여부: 해당하면 "장마철"이라고만 표현하고 월 숫자는 쓰지 않기\n'
        '  - 동절기 해당 여부: 해당하면 "동절기"라고만 표현하고 월 숫자는 쓰지 않기\n'
    )
    must_mention = "요일, 공차회송, 운행거리, 화물중량"
    if weather_summary:
        # ⚠️ 장마철/동절기 플래그는 "출발월이 6~8월/12~2월이면 무조건 참"인
        # 달력 규칙이라 실제 날씨와 안 맞을 수 있다(예: 장마철인데 실제로는
        # 맑음). 실제 기상 조회 결과가 있으면 그게 훨씬 정확한 정보이므로,
        # 이 부정확할 수 있는 계절 플래그는 프롬프트에서 아예 빼고 실제
        # 날씨로 대체한다 — 안 그러면 모델이 "비 소식 없음"인데도 그냥
        # "장마철"이라고만 언급해버리는 모순이 생긴다(실제로 관측된 문제).
        seasonal_bullets = ""
        weather_line = f"\n출발 시점 실제 기상 조회 결과(내부 참고, 모델 계산에는 미반영): {weather_summary}"
        weather_bullet = (
            "\n  - 기상 상황: 위 '출발 시점 실제 기상 조회 결과'를 바탕으로 표현하세요. "
            "비/눈 등 운송에 불리한 날씨면 그 점을 짧게 언급(예: \"비 소식도 있어\"), "
            "맑거나 무난하면 오히려 긍정 요인으로 언급(예: \"날씨도 무난해\")하거나 "
            "생략해도 됨. 기온·강수확률 등 구체적 수치는 여기서도 절대 쓰지 말 것. "
            "이 실제 기상 정보가 있으면, 이걸 계절감(장마철·동절기 등) 언급보다 우선하세요."
        )
        must_mention = "요일, 공차회송, 운행거리, 화물중량, 기상 상황"
    prompt = f"""아래는 화물열차 지연(운휴) 위험도를 학습된 모델이 예측한
결과입니다. 화주에게 "왜 이런 위험 등급이 나왔는지" 요인 중심으로 설명하는
문장을 존댓말로 작성하세요. 아래 숫자와 신호는 판단 근거로만 참고하고,
최종 설명 문구에는 구체적인 숫자를 절대 표시하지 마세요.

정성 신호를 빠짐없이, 딱 정해진 형식 없이 자연스러운 한두 문장(최대
세 문장)에 녹여서 설명하세요 — 아래 근거 신호에 있는 항목 중:
  - 요일: 요일 이름만 언급하고 운휴율 퍼센트는 쓰지 않기
  - 노선(주운행선)
{seasonal_bullets}  - 결합배송 여부
  - 공차회송 여부
  - 화물중량: 실제 톤 수치를 쓰지 말고 "가벼운 편", "무거운 편"처럼 표현
  - 운행거리: 실제 km 수치를 쓰지 말고 "짧은 편", "다소 긴 편"처럼 표현{weather_bullet}
이 중 이번 건에 실제로 해당하거나 영향이 큰 항목({must_mention})은
반드시 언급하세요. 해당하지 않는 계절 요인은 억지로 언급하지 말고
자연스럽게 생략해도 됩니다.
...
⚠️ 중요: "모델", "학습", "피처", "AI", "LightGBM" 같은 기술 용어는 절대
쓰지 마세요. 화주는 물류·IT 전문가가 아닙니다. 쉬운 일상어로, 마치 사람이
상황을 설명해주듯 자연스럽게 쓰세요.

아래 정보는 설명 작성용 내부 참고 정보이며, 여기에 포함된 숫자를 최종 문장에
그대로 옮기면 안 됩니다.
{feature_importance_note}
{weekday_base_rate_note}
