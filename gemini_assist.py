...
결과입니다. 화주에게 "왜 이 확률이 나왔는지" 요인 중심으로 설명하는
문장을 존댓말로 작성하세요. 주어진 숫자·신호·통계 외의 다른 수치·원인을
새로 만들어내지 마세요.
...
세 문장)에 녹여서 설명하세요 — 아래 근거 신호에 있는 항목 중:
  - 요일(+실측 운휴율 수치)
  - 노선(주운행선)
  - 장마철 해당 여부(6~8월인 경우만 언급, 아니면 생략)
  - 동절기 해당 여부(12~2월인 경우만 언급, 아니면 생략)
  - 결합배송 여부
  - 공차회송 여부
  - 화물중량, 운행거리
이 중 이번 건에 실제로 해당하거나(예: 장마철=1, 동절기=1) 영향이 큰
항목(요일, 공차회송, 운행거리, 화물중량)은 반드시 언급하세요. 해당 안
되는 계절 요인(예: 장마철 아님)은 굳이 "장마철이 아니라서 괜찮다"처럼
억지로 언급하지 말고 자연스럽게 생략해도 됩니다. 아래 영향도 순위를
참고해서 영향이 큰 항목을 우선 언급하세요.

⚠️ 중요: "모델", "학습", "피처", "AI", "LightGBM" 같은 기술 용어는 절대
쓰지 마세요. 화주는 물류·IT 전문가가 아닙니다. 초등학생도 알아들을 수
있는 쉬운 일상어로, 마치 사람이 상황을 설명해주듯 자연스럽게 쓰세요.
예를 들어 "학습 데이터상 위험도가 높게 나왔다" 대신 "요일과 거리를
고려했을 때 위험도가 다소 높게 예측됩니다"처럼 쓰세요.
