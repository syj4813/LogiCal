...
세 문장)에 녹여서 설명하세요 — 아래 근거 신호에 있는 항목 중:
  ...
  - 공차회송 여부
  - 화물중량, 운행거리
이 중 이번 건에 실제로 해당하거나(예: 장마철=1, 동절기=1) 피처 중요도가
높아 영향이 큰 항목(요일, 공차회송, 운행거리, 화물중량)은 반드시
언급하세요. 해당 안 되는 계절 요인(예: 장마철 아님)은 굳이 "장마철이
아니라서 괜찮다"처럼 억지로 언급하지 말고 자연스럽게 생략해도 됩니다.
아래 피처 중요도 순위를 참고해서 영향이 큰 항목을 우선 언급하세요.
