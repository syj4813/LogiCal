...
현재 실제 기사 로그인/GPS가 없기 때문에 기사-현재 화물역 매핑은
데모용 결정론적 값입니다. 실제 서비스에서는 로그인 사용자와 GPS로
교체하면 나머지 화면 로직은 그대로 사용할 수 있습니다.
"""
...
_DRIVER_NAMES = ["김도현", "이수민", "박정우", "최은서", "장민호"]
_STATION_NAMES = list(STATIONS.keys())
DRIVER_CURRENT_STATION = {
    name: _STATION_NAMES[i % len(_STATION_NAMES)] for i, name in enumerate(_DRIVER_NAMES)
}
...
k1.metric("현재 기준 화물역", my_station)
